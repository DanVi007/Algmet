...
bool storreEnnBarna(Node *node)
{
    ...
    if (node->left) // sjekker om venstre barn eksisterer
    {
        ...
        {
            largerChildren= storreEnnBarna(node->left);
        }
    }

    if (node->right && largerChildren) // sjekker om venstre barn eksisterer og om venstre siden til treet fort
    {
        ...
        {
            largerChildren= storreEnnBarna(node->right); // ellers fortsetter koden og sjekke storreEnnBarna for høyre barnet.
        }
    }

    return largerChildren; // returnere om venstre side og høyre side er har barn større enn foreldre.
}